...
results_path = "/home/careinfolab/Dr_Luo/Rohan/Gen_Adver_Nets/Results/WGAN_GP_DCNN"
...
def objective(trial):
    ...
    lam = trial.suggest_float("lambda", 5.0, 15.0)
    ...
    critic = DCCritic(im_chan=channels_dim, hidden_dim=64).to(device)

    _, loss_g = train(generator, critic, latent_dim, dataloader=dataloader, num_epochs=num_epochs, lr=learning_rate, device=device,k=k,results_path=results_path,isLinear=False,lam=lam)
...
best_critic = DCCritic(im_chan=channels_dim, hidden_dim=64).to(device)
...
train_loss_d, train_loss_g = train(best_generator, best_critic,latent_dim=best_params["latent_dim"],dataloader=best_dataloader,num_epochs=best_params["num_epochs"],lr=best_params["learning_rate"],device=device,k=5,results_path=results_path,isLinear=False,lam=best_params['lambda'])

torch.save(best_generator.state_dict(), os.path.join(results_path, "generator.pth"))
torch.save(best_critic.state_dict(), os.path.join(results_path, "discriminator.pth"))
