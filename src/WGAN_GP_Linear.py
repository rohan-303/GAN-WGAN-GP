...
def objective(trial):
    ...
    critic = LinearCritic(hidden_dim=hidden_dim, num_layers=num_layers,in_dim=pixels*channels_dim).to(device)
...
best_critic = LinearCritic(hidden_dim=best_params["hidden_dim"],num_layers=best_params["num_layers"],in_dim=pixels*channels_dim).to(device)
...
train_loss_d, train_loss_g = train(best_generator, best_critic,latent_dim=best_params["latent_dim"],dataloader=best_dataloader,num_epochs=best_params["num_epochs"],lr=best_params["learning_rate"],device=device,k=5,results_path=results_path,lam=best_params['lambda'])
